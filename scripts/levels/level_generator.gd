...
func generate_room(difficulty: int) -> TileMap:
	...
	player_spawn.position = (
		Vector2i(2, y_tiles / 2) * tile_map.tile_set.tile_size + tile_map.tile_set.tile_size / 2
	)
	room.add_child(player_spawn.duplicate())
	player_spawn.name = "East Spawn"
	player_spawn.position = (
		Vector2i(x_tiles - 3, y_tiles / 2) * tile_map.tile_set.tile_size
		+ tile_map.tile_set.tile_size / 2
	)
	room.add_child(player_spawn.duplicate())
	player_spawn.name = "North Spawn"
	player_spawn.position = (
		Vector2i(x_tiles / 2, 2) * tile_map.tile_set.tile_size + tile_map.tile_set.tile_size / 2
	)
	room.add_child(player_spawn.duplicate())
	player_spawn.name = "South Spawn"
	player_spawn.position = (
		Vector2i(x_tiles / 2, y_tiles - 8) * tile_map.tile_set.tile_size
		+ tile_map.tile_set.tile_size / 2
	)
	...
	while obstacles.size() < objective_max && iterations < 10000:
		...
		var obstacle = get_pattern_rect(
			Vector2(randi_range(0, x_tiles), randi_range(0, y_tiles)), half_pattern_size
		)
		if (
			obstacles.any(func(o): return o.intersects(obstacle))
			|| room_borders.any(func(b): return b.intersects(obstacle))
		):
			continue
	...
	# Place doors
	print("Placing Doors")
	var door_scene = load("res://scenes/doors/door_a.tscn")
	var door = door_scene.instantiate()
	room.room_complete.connect(door._on_room_complete)
	door.position = (
		Vector2i(0, y_tiles / 2) * tile_map.tile_set.tile_size + tile_map.tile_set.tile_size / 2
	)
	door.name = "West Door"
	room.add_child(door)
	...
	door.rotate(PI)
	door.position = (
		Vector2i(x_tiles - 1, y_tiles / 2) * tile_map.tile_set.tile_size
		+ tile_map.tile_set.tile_size / 2
	)
	door.name = "East Door"
	room.add_child(door)
	...
	door.rotate(PI / 2)
	door.position = (
		Vector2i(x_tiles / 2, 0) * tile_map.tile_set.tile_size + tile_map.tile_set.tile_size / 2
	)
	...
	door.rotate(-PI / 2)
	door.position = (
		Vector2i(x_tiles / 2, y_tiles - 6) * tile_map.tile_set.tile_size
		+ tile_map.tile_set.tile_size / 2
	)
...
func get_pattern_rect(position: Vector2, half_pattern_size: Vector2) -> Rect2i:
	var obstacle = Rect2i(
		floor(position - half_pattern_size - Vector2(1, 1)), half_pattern_size * 2 + Vector2(1, 1)
	)
